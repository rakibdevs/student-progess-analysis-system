   ...
   if($_SESSION['category']=='Teacher'){
      $query_e = $mysqli->query("select * from exam where teacher_id=$teacher_id and status='Running' order by exam_date desc ");
      }
      if($_SESSION['category']=='Student'){
        $query_e = $mysqli->query("select * from exam inner join course on exam.course_id=course.id where  exam.status='Running' and exam.exam_session='$sExam_session' and course.lvl_sem='$sLvl_sem' order by exam.exam_date desc");
      }
      $numrow=$query_e->num_rows;

    if ($numrow>0) { ?>
    <div class="panel-body row">
          <center><b>Running Exam</b></center>
    ...
    <li>
      <div class="<?php if (($userCat=='Teacher')||$userCat=='Student') { echo 'col-lg-9';}else { echo'col-lg-12';}?>" style="padding: 0;">
       ...
       <div class="col-lg-3" style="text-align:right;padding: 0;">
        ...
        <button data-toggle="tooltip" title="Start Exam" class="pd-setting-ed" type="button" style="color:green;" ><a style="color:green;" href="take_exam.php?id=<?php echo $exam['exam_id']; ?>&&Key=<?php echo $exam['exam_key']; ?>">View</a> </button>
        <?php } ?>
        <?php if($_SESSION['category']=='Student'){ ?>
        <button data-toggle="tooltip" title="Participate in Exam" class="pd-setting-ed" type="button" style="color:green;" ><a style="color:green;" href="exam.php?id=<?php echo $exam['exam_id']; ?>&&Key=<?php echo $exam['exam_key']; ?>">Join</a> </button>
        <?php } ?>
        </div>
